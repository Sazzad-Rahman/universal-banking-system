<?php
    ...
    $regex_password = '/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/';
		...
		if(isset($_POST['reg']))
		{
			...
			{
				echo "<i style='color:white;background-color:red;'>Invalid Name! Try a valid name!</i><br>";
				$flag = 0;
			}
		...
		if($flag==1)
		{
		    ...
		    if ($conn->query($sql) === TRUE)
				...
				{
		    	echo "<i style='color:white;background-color:red;'>There were some problems in the server! Please try again after some time!</i><br>".$conn->error;
		    }
		}
	}
